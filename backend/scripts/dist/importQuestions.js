...
function getStructureError(type, field) {
...
async function upsertQuestion(questionData, createdBy) {
...
async function importQuestions(filePath, createdBy) {
    const dataPath = path_1.default.join(__dirname, filePath);
    const ext = path_1.default.extname(dataPath);
    let questions = [];
    try {
...
async function deleteQuestions(type, level) {
    const where = {};
    if (type) {
        where.metadata = {
            path: ['type'],
